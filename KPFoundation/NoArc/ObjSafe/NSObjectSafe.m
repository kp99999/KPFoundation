...
{
    if(!key){
        SFAssert(NO, @"hookhookSetValue invalid args: %@", self);
        return;
    }
    @try{
        [self hookSetValue:value forKey:key];
    }@catch(NSException *exception){
        NSLog(@"hookSetValue ex: %@", [exception callStackSymbols]);
    }
}
@end
...
- (void) hookSetValue:(id)anValue forKey:(id)aKey {
    if (anValue && aKey) {
        [self hookSetValue:anValue forKey:aKey];
    } else {
//        SFAssert(NO, @"NSMutableDictionary invalid args hookSetValue:[%@] forKey:[%@]", anValue, aKey);
    }
}
